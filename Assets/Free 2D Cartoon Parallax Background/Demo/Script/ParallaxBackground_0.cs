...
public class ParallaxBackground_0 : MonoBehaviour
{
    ...
    private float sizeX;
    private readonly float[] startPos = new float[7];
    ...
    private void Start()
    {
        ...
        for (var i = 0; i < 5; i++) startPos[i] = _camera.position.x;
    }

    private void Update()
    {
        //Moving camera
        if (Camera_Move) _camera.position += Vector3.right * Time.deltaTime * Camera_MoveSpeed;
        for (var i = 0; i < 5; i++)
        {
            var temp = _camera.position.x * (1 - Layer_Speed[i]);
            var distance = _camera.position.x * Layer_Speed[i];
            ...
            else if (temp < startPos[i] - boundSizeX * sizeX) startPos[i] -= boundSizeX * sizeX;
        }
    }
}